...
class CreatorPlatformController extends Controller
{
    ...
    public function getTwitterFollowers(string $profileUrl)
    {
        try {
            $client = new Client();
            ...
            // Example: Extract follower count (adjust selectors based on Twitter's HTML)
            $followersCount = $crawler->filter('[data-testid="follower_count"]')->text();
            $tweetsCount = $crawler->filter('[data-testid="tweet_count"]')->text();
            dd((int) preg_replace('/[^0-9]/', '', $followersCount), (int) preg_replace('/[^0-9]/', '', $tweetsCount)
            return [
                'followers_count' => (int) preg_replace('/[^0-9]/', '', $followersCount),
                'tweets_count' => (int) preg_replace('/[^0-9]/', '', $tweetsCount),
            ];
        } catch (\Exception $e) {
            return null;
        }
