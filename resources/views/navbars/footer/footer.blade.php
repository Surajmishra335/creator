...
<script>
    ...
    $("#add_new_social_platform").click(function() {
        // Get dynamic data if needed (e.g., creator_id or platform_id)

        $.ajax({
            url: "{{ route('social-platform-create') }}", // Named route for your Blade view
            type: "GET",
            success: function(response) {
                // Append the response (Blade file content) to the modal container
                $("#add_social_platforms").html(response);

                // Optionally open the modal if you're using Bootstrap
                $('#add_social_platforms').modal('show');
            },
            error: function(xhr) {
